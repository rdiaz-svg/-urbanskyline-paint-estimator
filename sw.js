const CACHE_NAME = 'urbanskyline-v8.3.0-guided-workflow';
const APP_SHELL = [
  './?usl_pwa=810',
  './index.html',
  './styles.css?v=830',
...
self.addEventListener('activate', event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith('urbanskyline-') && k !== CACHE_NAME).map(k => caches.delete(k)))).then(() => self.clients.claim()));
});
