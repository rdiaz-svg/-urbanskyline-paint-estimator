const CACHE_NAME = 'urbanskyline-v8.0.3';
const APP_SHELL = ['./','./index.html','./styles.css?v=803','./app.js?v=803','./manifest.json','./urban-skyline-logo.png'];
...
self.addEventListener('fetch', event => {
  ...
  if(url.pathname.endsWith('/version.json') || url.pathname.endsWith('/sw.js')) { event.respondWith(fetch(event.request, {cache:'no-store'})); return; }
