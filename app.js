...
window.uslCloudBackup=async function(){const msg=$("syncMessage"),btn=$("pushSheet");try{if(msg){msg.textContent='Preparing complete cloud backup…';msg.className='sync-status working'}if(btn){btn.disabled=true;btn.textContent='Backing Up…'}if(state.workflow?.mode==='project'&&state.workflow?.approvedSnapshot)archiveCurrentProject();const photos=await exportCloudPhotos();const payload={schemaVersion:1,appVersion:'7.3.1',savedAt:new Date().toISOString(),state:stateForArchive(state),history:loadHistory(),photos};const d=await uslApi('cloudBackup',{backup:payload});if(msg){msg.textContent=`Cloud backup complete${d.savedAt?' · '+new Date(d.savedAt).toLocaleString():''}. ${photos.length} photo${photos.length===1?'':'s'} included.`;msg.className='sync-status success'}}catch(e){if(msg){msg.textContent='CLOUD BACKUP FAILED: '+(e.message||e);msg.className='sync-status error'}}finally{if(btn){btn.disabled=false;btn.textContent='Back Up Cloud'}}};
