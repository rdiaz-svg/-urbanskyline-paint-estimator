...
window.uslCloudBackup=async function(){const msg=$("syncMessage"),btn=$("pushSheet");try{if(msg){msg.textContent='Preparing complete cloud backup…';msg.className='sync-status working'}if(btn){btn.disabled=true;btn.textContent='Backing Up…'}if(state.workflow?.mode==='project'&&state.workflow?.approvedSnapshot)archiveCurrentProject();const photos=await exportCloudPhotos();const payload={schemaVersion:1,appVersion:'7.3.0',savedAt:new Date().toISOString(),state:stateForArchive(state),history:loadHistory(),photos};const d=await uslApi('cloudBackup',{backup:payload});if(msg){msg.textContent=`Cloud backup complete${d.savedAt?' · '+new Date(d.savedAt).toLocaleString():''}. ${photos.length} photo${photos.length===1?'':'s'} included.`;msg.className='sync-status success'}}catch(e){if(msg){msg.textContent='CLOUD BACKUP FAILED: '+(e.message||e);msg.className='sync-status error'}}finally{if(btn){btn.disabled=false;btn.textContent='Back Up Cloud'}}};
window.uslCloudRestore=async function(){const msg=$("syncMessage"),btn=$("pullSheet");try{if(!confirm('Restore the latest UrbanSkyLine cloud backup on this device? The current local estimate will be replaced.'))return;if(msg){msg.textContent='Restoring latest cloud backup…';msg.className='sync-status working'}if(btn){btn.disabled=true;btn.textContent='Restoring…'}const d=await uslApi('cloudRestore');if(!d.backup||!d.backup.state)throw Error('No cloud backup was found.');state=d.backup.state;if(!state.project.photoProjectId)state.project.photoProjectId='p_'+Date.now()+'_'+Math.random().toString(36).slice(2,8);localStorage.setItem('uslPaintApp',JSON.stringify(state));saveHistory(Array.isArray(d.backup.history)?d.backup.history:[]);await replaceProjectPhotosFromCloud(d.backup.photos||[]);bindProject();bindPhotoInputs();bindSubcontractor();renderRooms();renderCabinets();renderColors();refreshAll();await renderProjectPhotos();if(msg){msg.textContent='Cloud restore complete. Full project data and photos are now on this device.';msg.className='sync-status success'}}catch(e){if(msg){msg.textContent='CLOUD RESTORE FAILED: '+(e.message||e);msg.className='sync-status error'}}finally{if(btn){btn.disabled=false;btn.textContent='Restore Cloud'}}};
