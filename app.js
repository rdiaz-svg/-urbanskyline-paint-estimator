...
function refreshAll(){renderWorkflow();
  ...
  const cabinetFinishMap=new Map();let cabinetPrimerBuy=0;
  (state.cabinets||[]).forEach(cab=>{if(cab.selected===false)return;const q=cabinetCalc(cab);if(q.hours<=0)return;const key=(cab.color||'Cabinet Color')+'|'+(cab.sw||'');const prev=cabinetFinishMap.get(key)||{color:cab.color||'Cabinet Color',sw:cab.sw||'',buyGal:0};prev.buyGal+=q.finishGal;cabinetFinishMap.set(key,prev);cabinetPrimerBuy+=q.primerGal;});
  const cm=materialSettings();
  const cabinetMaterialHtml=[...cabinetFinishMap.values()].map(g=>`<div class="material-row cabinet-material-row"><span>Cabinet Finish<br><small>${g.color}${g.sw?' • '+g.sw:''}</small></span><span>${cm.trimProduct}<br><small>Specialty cabinet coating • 2 finish coats • 10% waste included</small></span><strong>Buy ${g.buyGal} gal<br><small>${money2(cm.trimCost)}/gal · ${money2(g.buyGal*cm.trimCost)}</small></strong></div>`).join('')+(cabinetPrimerBuy?`<div class="material-row cabinet-material-row"><span>Cabinet Primer<br><small>Bonding / prep primer</small></span><span>${cm.primerProduct}<br><small>1 primer coat • 10% waste included</small></span><strong>Buy ${cabinetPrimerBuy} gal<br><small>${money2(cm.primerCost)}/gal · ${money2(cabinetPrimerBuy*cm.primerCost)}</small></strong></div>`:'');
