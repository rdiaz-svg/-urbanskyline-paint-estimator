...
function scopeValue(r,key){
  ...
  if(key==="walls") return r.package==="Full Room" || r.package==="Walls Only";
...
function renderRooms(){const w=$('roomList');w.innerHTML='';state.rooms.forEach((r,i)=>{
  ...
  <div class="scope-summary">Included: ${scopeSummary(r)}<br><small>Paintable: ${Math.round(q.includedWallSf)} wall/closet SF • ${Math.round(q.includedCeilingSf)} ceiling SF • ${Math.round(q.includedTrimEqSf)} trim-equivalent SF</small><br><small>Quantities: ${include(r,'baseboards')?Math.round(q.baseLf)+' baseboard LF':'0 baseboard LF'} • ${include(r,'doors')?q.doors:0} doors • ${include(r,'windows')?q.windows:0} windows • ${include(r,'crown')?Math.round(q.crownLf)+' crown LF':'0 crown LF'}</small></div>`;w.appendChild(d)});
  ...
  w.querySelectorAll('input,select').forEach(e=>e.onchange=()=>{const r=state.rooms[+e.dataset.i],k=e.dataset.k;r[k]=e.type==='number'?Math.max(0,Number(e.value)):e.value;if(k==='package'){if(e.value==='Full Room'){r.walls='Auto';r.ceiling='Auto';r.baseboards='Auto';r.doors='Auto';r.windows='Auto';r.closets='No';r.crown='No'}else if(e.value==='Walls Only'){r.walls='Auto';r.ceiling='Auto';r.baseboards='Auto';r.doors='Auto';r.windows='Auto';r.closets='No';r.crown='No'}else if(e.value==='Custom'){r.walls='No';r.ceiling='No';r.baseboards='No';r.doors='No';r.windows='No';r.closets='No';r.crown='No'}}save();renderRooms();renderColors()})}
...
function componentHours(r,q=roomQty(r)){
  return {
    ...
    closets:(include(r,'closets')?q.closetSf:0)/150,
...
function proposalScope(r){const q=roomQty(r),items=[];if(include(r,'walls'))items.push(`Walls — ${Math.round(q.wallSf)} SF`);if(include(r,'ceiling'))items.push(`Ceiling — ${Math.round(q.ceilingSf)} SF`);if(include(r,'baseboards'))items.push(`${Math.round(q.baseLf)} LF baseboards`);if(include(r,'doors'))items.push(plural(q.doors,'interior door','interior doors'));if(include(r,'windows'))items.push(plural(q.windows,'window','windows'));if(include(r,'closets'))items.push(`Closet walls — ${Math.round(q.closetSf)} SF`);if(include(r,'crown'))items.push(`${Math.round(q.crownLf)} LF crown molding`);return items.join('<br>')||'No work selected'}
function proposalPaint(r){const lines=[];if(include(r,'walls')||include(r,'closets'))lines.push(`Walls: ProMar 200 Eggshell — ${r.wallColor||'Color TBD'}${r.wallSw?' ('+r.wallSw+')':''}`);if(include(r,'ceiling'))lines.push(`Ceiling: ProMar 200 Flat — ${r.ceilingColor||'Color TBD'}${r.ceilingSw?' ('+r.ceilingSw+')':''}`);if(include(r,'baseboards')||include(r,'doors')||include(r,'windows')||include(r,'crown'))lines.push(`Trim: Emerald Urethane Trim Enamel — ${r.trimColor||'Color TBD'}${r.trimSw?' ('+r.trimSw+')':''}`);return lines.join('<br>')}
