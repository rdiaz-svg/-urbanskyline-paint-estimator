...
const PRICING={minimumJob:250,targetMargin:0.40,materialPerGallon:48,painterDayRate:300,hoursPerDay:8,setupCleanupPct:0.15,wastePct:0.10};
...
function refreshAll(){
  ...
  $('materialSummary').innerHTML=c.groups.length?c.groups.map(g=>`<div class="material-row"><span>${g.surface}<br><small>${g.color}${g.sw?' • '+g.sw:''}</small></span><span>${g.product}<br><small>${Math.round(g.sf)} sq ft • 2 coats • ${g.coverage} sq ft/gal</small><br><small>${g.baseGal.toFixed(2)} gal coating + 10% waste = <strong>${g.calcGal.toFixed(2)} gal required</strong></small></span><strong>Buy ${g.buyGal} gal</strong></div>`).join('')+'<p class="muted material-note">Paint is combined by matching product and color before rounding to whole gallons. Coverage assumption: 400 sq ft/gal; 2 coats; 10% waste.</p>':'<p class="muted">Select rooms to calculate materials.</p>';
